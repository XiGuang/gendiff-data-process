...
from utils import import_obj_files,perform_boolean_intersection,create_cutting_cube,export_object_as_obj,clear_scene
...
def get_may_change_models(t1_yaml:str):
    ...
    return new_building_models, position, size
...
def export_clipped_obj(mesh, save_dir, cube_center, cube_size):
    ...
    cutting_cube = create_cutting_cube(cube_size, cube_center)
    retained_mesh = perform_boolean_intersection(copied_obj, cutting_cube)

    if retained_mesh.data.vertices.__len__() == 0:
        print(f"模型 {retained_mesh.name} 在立方体内无顶点，跳过导出")
        bpy.data.objects.remove(cutting_cube, do_unlink=True)
...
def process_change_model(yaml: str, save_dir: str, building_models, sample_points_num: int = 1000, workers: int | None = None):
    new_building_models, position, size = get_may_change_models(yaml)
...
if __name__ == "__main__":
    ...
    for f in folders:
        ...
        if not os.path.exists(yaml_file):
            ...
            continue
        process_change_model(yaml_file, f, building_models, sample_points_num, workers)
    print("处理完成")
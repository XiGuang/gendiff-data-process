...
from ..types import CanonicalBuildingSequence, CanonicalEdit, CanonicalLayer, CanonicalStage, PointEdit
...
class AreaNormalizationStats:
    ...
    def normalize_xz(self, point_q: tuple[int, int], grid_xz: str) -> list[float]:
        ...
        return [(x - self.center_x) / self.scale_xz, (z - self.center_z) / self.scale_xz]
    ...
    def normalize_xyz(self, point_q: tuple[int, int, int], grid_xz: str, grid_y: str) -> list[float]:
        x = float(dequantize_scalar(point_q[0], grid_xz))
...
class AreaV2Adapter:
    ...
    def _proxy_id(dense_building_id: int, layer_lineage_id: int) -> int:
        if not 0 <= layer_lineage_id < 10_000:
            raise CanonicalizationError("E_ID_OVERFLOW", "layer lineage 超出 area-v2 stride")
        return dense_building_id * 1_000_000_000 + layer_lineage_id

    @staticmethod
    def _point_id(dense_building_id: int, layer_lineage_id: int, point_lineage_id: int) -> int:
        if not 0 <= point_lineage_id < 100_000:
            raise CanonicalizationError("E_ID_OVERFLOW", "point lineage 超出 area-v2 stride")
        return (
    ...
    def _layer(self, layer: CanonicalLayer, dense_building_id: int) -> dict:
        if layer.layer_lineage_id is None or len(layer.point_lineage_ids) != len(layer.footprint_q):
            raise ValueError("area-v2 adapter 只接受已分配完整 lineage 的 canonical layer")
        ...
        return {
            ...
            "min_height": self.normalization.normalize_y(layer.min_height_q, self.bundle.canonicalizer.grid_y),
            "max_height": self.normalization.normalize_y(layer.max_height_q, self.bundle.canonicalizer.grid_y),
            "footprint": [
                self.normalization.normalize_xz(point, self.bundle.canonicalizer.grid_xz)
                for point in layer.footprint_q
            ],
            "point_ids": [self._point_id(dense_building_id, lineage, point_id) for point_id in layer.point_lineage_ids],
            "source_point_ids": [
                self._point_id(dense_building_id, lineage, point_id) for point_id in layer.point_lineage_ids
            ],
    ...
    def adapt_state(self, stage: CanonicalStage, dense_building_id: int, building_key: str) -> dict:
        if not 0 <= dense_building_id < self.capacity.max_buildings:
            raise CanonicalizationError("E_BUILDING_CAPACITY", "dense building ID 超出 tile 容量")
        if len(stage.layers) > self.capacity.max_layers:
            raise CanonicalizationError("E_LAYER_CAPACITY", "adapter 输入 stage 超过层容量")
        return {
    ...
    def _point_entry(
        ...
        if source_layer is not None and point_edit.source_index is not None:
            source_coord = self.normalization.normalize_xz(
                source_layer.footprint_q[point_edit.source_index], self.bundle.canonicalizer.grid_xz
            )
        target_coord = None
        if target_layer is not None and point_edit.target_index is not None:
            target_coord = self.normalization.normalize_xz(
                target_layer.footprint_q[point_edit.target_index], self.bundle.canonicalizer.grid_xz
            )
    ...
    def adapt_edit(
        ...
        insert_count = sum(layer_edit.action == "INSERT_LAYER" for layer_edit in edit.layer_edits)
        if len(source.layers) + insert_count > self.capacity.max_layers:
            raise CanonicalizationError("E_LAYER_CAPACITY", "source slots 加 insert slots 超过模型容量")
        ...
        for layer_edit in edit.layer_edits:
            ...
            point_entries = [
                ...
                if (entry := self._point_entry(point_edit, source_layer, target_layer)) is not None
            ]
            if len(point_entries) > self.capacity.max_points_per_layer:
                raise CanonicalizationError("E_POINT_CAPACITY", "adapter point edit 超过 AR token 容量")
            ...
            source_height = (
                [
                    self.normalization.normalize_y(source_layer.min_height_q, self.bundle.canonicalizer.grid_y),
                    self.normalization.normalize_y(source_layer.max_height_q, self.bundle.canonicalizer.grid_y),
                ]
                if source_layer is not None
                else None
            )
            target_height = (
                [
                    self.normalization.normalize_y(target_layer.min_height_q, self.bundle.canonicalizer.grid_y),
                    self.normalization.normalize_y(target_layer.max_height_q, self.bundle.canonicalizer.grid_y),
                ]
                if target_layer is not None
                else None
            )
            output.append(
                {
                    ...
                    "source_building_id": dense_building_id if source_layer is not None else None,
                    "target_building_id": dense_building_id if target_layer is not None else None,
    ...
    def adapt_pair(
        ...
        *,
        ...
        condition = [[float(point[0]), float(point[1]), float(point[2])] for point in condition_points_normalized]
        ...
        if any(len(point) != 3 or not all(math.isfinite(value) for value in point) for point in condition):
            raise CanonicalizationError("E_NONFINITE_VALUE", "adapter condition 必须是有限 XYZ 三元组")
        canonical_metadata = {
        ...
        return {
            "source_state": self.adapt_state(source, dense_building_id, sequence.building_key),
            "target_state": self.adapt_state(target, dense_building_id, sequence.building_key),
            "sample": {
                ...
                "validation": {"canonical_roundtrip": True, "canonical_metadata": canonical_metadata},
